...
#########################
###### 3D Tag position  #####
X=np.array([7.65,0.13,2.7,0,0.9,2.75,0.17,11.8,2.72,7.78,11.7,2.5]).reshape([4,3])
Y=(0,5.64,2.73)
def spread_2D():
...
def EKF_message():
    ...
    #if (tag[0] and tag[1] and tag[2] and tag[3])!=0:
    EKF_Update()
def EKF_New():
...
def EKF_Update():
    ##### IMU_MPU9250 #####
    global tag,accel,gyro,compass,Grid,n_ekf,dwm,Imu,state,xpm_Nh,ypm_Nh,xvm_Nh,yvm_Nh,xam_Nh,yam_Nh,wzm_h,bx_h,by_h,bz_h,psi_h,end_count,P00_z,tmp,tmp_1,tmp_YX,u,select,count1,select_d,EKF_Solution_Anc,R,Xz_h,F_z,Q_z,H_D,bx_h_1,by_h_1,bz_h_1,psi_h_1,xpm_Nh_1,ypm_Nh_1,xvm_Nh_1,yvm_Nh_1,xam_Nh_1,yam_Nh_1
	...
	for j in range(0,2):
    	...
    	R1m_h = ((X[1,0]-xpm_Nh)**2+(X[1,1]-ypm_Nh)**2+(X[1,2]-1.32)**2)**(0.5)
    	R2m_h = ((X[2,0]-xpm_Nh)**2+(X[2,1]-ypm_Nh)**2+(X[2,2]-1.32)**2)**(0.5)
    	R3m_h = ((X[3,0]-xpm_Nh)**2+(X[3,1]-ypm_Nh)**2+(X[3,2]-1.32)**2)**(0.5)
    	R4m_h = ((X[0,0]-xpm_Nh)**2+(X[0,1]-ypm_Nh)**2+(X[0,2]-1.32)**2)**(0.5)
    	##### H Matrix Residual Calculator #####
    	r1_partial_x =-(X[1,0]-xpm_Nh)/R1m_h
    	r1_partial_y =-(X[2,1]-ypm_Nh)/R1m_h
    	r2_partial_x =-(X[2,0]-xpm_Nh)/R2m_h
    	r2_partial_y =-(X[2,1]-ypm_Nh)/R2m_h
    	r3_partial_x =-(X[3,0]-xpm_Nh)/R3m_h
    	r3_partial_y =-(X[3,1]-ypm_Nh)/R3m_h
    	r4_partial_x =-(X[0,0]-xpm_Nh)/R4m_h
    	r4_partial_y =-(X[0,1]-ypm_Nh)/R4m_h
	...
	print("--------------------------------------------------save--------------------------------------------")
        print("--------------------------------------------------save--------------------------------------------")
        print("--------------------------------------------------save--------------------------------------------")
	np.savetxt('bias_data_0202.csv',(bias_array) ,delimiter=',')
...
def loop():
    global Anchor,current_anchor,uwb_array,sentAck,n_ekf_start,start,n_23,n_24,n_25,n_26,n_27,n_29,receivedAck, timePollAckSentTS, timePollReceivedTS, timePollSentTS, timePollAckReceivedTS, \
	timeRangeReceivedTS, protocolFailed, data, expectedMsgId,expectedMsgID, timeRangeSentTS,Position_Flag,DistanceFinish_Flag,Same_tag_flag,EKF_start,EKF_message,EKF_New,EKF_Update
    ...
    if Position_Flag==0:
		...
		if (DistanceFinish_Flag == 1 and Same_tag_flag == data[16]): 
			...
			if data[16]==29:
                            ...
                            if distance <12 and distance >5:
                                tag[5]=distance
 				#uwb_array[5,n_29]=tag[5]
				#print(uwb_array[5,n_29],n_29)
				n_29+=1
			if n_29==4000 : #and n_25>10 and n_26>10 and n_27>10 and n_29>10:
			    #print("distance----------------save----------------------")
                            #print("distance----------------save----------------------")
                            #print("distance----------------save----------------------")
                            #np.savetxt('6_distance_data_1.csv',(uwb_array) , delimiter=',')
			    pass
			"""
                else:
                    transmitRangeFailed()
